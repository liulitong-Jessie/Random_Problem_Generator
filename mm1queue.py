...
class MM1Queue(Model):
    ...
    def get_warmup(self, random_rng, m = 50):
        self.rand_fuc = True
        warm = random_rng.expovariate(1/m)
        return int(warm)
        
    def get_people(self, random_rng, m = 100):
        self.rand_fuc = True
        people = random_rng.expovariate(1/m)
        return int(people)
    ...
    def attach_rng(self, random_rng):
        ...
        self.factors["warmup"] = self.get_warmup(random_rng[0])
        self.factors["people"] = self.get_people(random_rng[1])
        print('warmup: ', self.factors["warmup"])
        print('people: ', self.factors["people"])
...
class MM1MinMeanSojournTime(Problem):
    ...
    def __init__(self, name="MM1-1", fixed_factors=None, model_fixed_factors=None, random=False, random_rng=None):
        ...
        self.model = MM1Queue(self.model_fixed_factors, random)
