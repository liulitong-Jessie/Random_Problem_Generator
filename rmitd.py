...
class RMITD(Model):
    ...
    def get_means(self, random_rng):
        self.rand_fuc = True
        demand = [random_rng.expovariate(1/50), random_rng.expovariate(1/20), random_rng.expovariate(1/30)]
        return demand
        
    def get_cost(self, random_rng):
        self.rand_fuc = True
        cost = random_rng.expovariate(1/80)
        self.factors["cost"] = cost
        return cost
        
    def get_prices(self, random_rng):
        #cost should be smaller than prices 100 300 400
        self.rand_fuc = True
        prices = []
        ind = [100, 300, 400]
        for i in range(3):
            p = 0
            while self.factors["cost"] >= p:
                p = random_rng.expovariate(1/ind[i])
                # print('price: ', p)
            prices.append(p)
    ...
    def attach_rng(self, random_rng):
        ...
        self.factors["demand_means"] = self.get_means(random_rng[0])
        self.factors["cost"] = self.get_cost(random_rng[1])
        self.factors["prices"] = self.get_prices(random_rng[2])
        print('demands: ', self.factors["demand_means"])
        print('cost: ', self.factors["cost"])
        print('Prices: ', self.factors["prices"])
...
class RMITDMaxRevenue(Problem):
    ...
    def __init__(self, name="RMITD-1", fixed_factors=None, model_fixed_factors=None, random=False, random_rng=None):
        ...
        self.model = RMITD(self.model_fixed_factors, random)
